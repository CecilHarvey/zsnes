...
static void handle_params(int argc, char *argv[])
{
  ...
  */
	...
	i = 1;
#ifdef __MACOSX__
	// Mac OS X will give us a -psn_0_xxx argument when launched from Finder
	// So ignore it if it's there, since we can't do anything with it
	if (argc >= 2) {
		if (argv[1][0] == '-' && argv[1][1] == 'p' && argv[1][2] == 's' &&
			argv[1][3] == 'n' && argv[1][4] == '_') {
			i = 2;
		}
	}
#endif
...
void zmain(int zargc, char *zargv[])
{
	...
	// For now, also dump the command line args
	fprintf(stderr, "ZSNES arguments:\n");
